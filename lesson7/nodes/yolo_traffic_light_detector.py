...
class YoloTrafficLightDetector:
    ...
    def local_path_callback(self, local_path_msg):
        ...
        if len(local_path_msg.waypoints) >= 2: #check if there are at least 2 waypoints to form a path
            path_linestring = shapely.LineString([
                (
                    waypoint.position.x,
                    waypoint.position.y
                )
    ...
    def camera_image_callback(self, camera_image_msg):
        ...
        if stop_line_ids_on_path:
            ...
            if map_rois:
                ...
                # Keep only the main traffic light classes and discard
                # arrow-specific detections.
                mask = classes < 4
        ...
        for stop_line_id in stop_line_ids_on_path:
            for traffic_light_id, traffic_light_coords in self.traffic_lights[stop_line_id].items():
                ...
                for x, y, z in traffic_light_coords:
                    ...
                    if point_camera.z <= 0: #traffic light not visible
                        break
    ...
    def match_map_and_yolo_rois(self, map_rois, yolo_rois, yolo_classes, yolo_scores):
        ...
        for stop_line_id, traffic_light_id, x1_map, x2_map, y1_map, y2_map in map_rois:
            ...
            for idx, (cls, score, yolo_roi) in enumerate(
                ...
                #skip detection with iouscore below threshold
                if iou_score < self.iou_threshold:
                    continue
                #keep detection with highest iou score
                if matched_roi is None or iou_score > best_iou:
                    best_iou = iou_score
            ...
            if matched_roi is None:
                ...
                cls, score, _, _ = matched_roi #cls, score, yolo_roi, idx
                cls = int(cls) #convert to int

                tfl_result.status = CLASS_TO_TLRESULT[cls]
                tfl_result.status_text = CLASS_TO_STRING[cls]
                match_dict[traffic_light_id] = matched_roi

            # Both matched and missing traffic lights must be published.
            tfl_results.append(tfl_result)

        # Return only after every map ROI has been processed.
